...
// Basic Web Server
// CMS450, Fall 2020
//FOr some reason I cant get the jpeg to work
...
if (fd ==NULL){
   send_error_response(socket_fd, "404", "File not found", "Server cannot find the file");
    pthread_exit(0);
}
  // Stat the file to learn its size
struct stat buf1;
stat(filename, &buf1);
filesize = buf1.st_size;
 ...
 char response[MAX_LINE];
                ...
                sprintf(response, "%s 200 OK \r\nServer: %s\r\nContent-Length: %d\r\nContent-Type: %s\r\n\n", version, "450 Webserver", filesize, get_filetype(filename));
...
if (strcmp(get_filetype(filename),"image/jpeg")==0){
    ...
    while(filesize>0){
        i= send(socket_fd,point, sizeof(point),0);
