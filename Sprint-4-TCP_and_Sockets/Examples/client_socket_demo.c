...
// 
// Client side socket -- active open socket , wants to initiate a connection
// Server Side socket -- passive open, wants to wait to receive incoming client connections


// Here in the client, we're interested in creating a socket, connecting it to a server, and then sending messages and possibly receiving responses

// Server side will create a socket, bind it to a port, and then listen for incoming connections
...
int main(int argc, char** argv) {
  ...
  // client_fd is a client socket "file descriptor"
  // int that refers to a table of open socket connections manage by the OS
  ...
  // Key ideas:
  // Sequence of functions you need: create socket ----> connect socket ----> write/read (send/recv)
